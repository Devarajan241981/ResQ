...
const URGENCY_KEYS: Record<Urgency, TranslationKey> = {
  normal: "bloodDonation.form.urgencyNormal",
  urgent: "bloodDonation.form.urgencyUrgent",
  critical: "bloodDonation.form.urgencyCritical",
};
...
export function RequestForm() {
  ...
  return (
    <div className="mx-auto max-w-md">
      <h1 className="text-2xl font-semibold">{t("bloodDonation.form.heading")}</h1>
      ...
      <form onSubmit={handleSubmit} className="mt-4 flex flex-col gap-3">
        <label className="flex flex-col gap-1 text-sm">
          {t("bloodDonation.form.patientName")}
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="flex flex-col gap-1 text-sm">
            {t("bloodDonation.form.bloodGroup")}
          ...
          <label className="flex flex-col gap-1 text-sm">
            {t("bloodDonation.form.unitsNeeded")}
        ...
        <button type="submit" disabled={isSubmitting} className="mt-2 rounded-md bg-foreground px-4 py-2 text-background disabled:opacity-50">
          {isSubmitting ? t("bloodDonation.form.posting") : t("bloodDonation.form.post")}
        </button>
      </form>
    </div>
  );
