...
class IsVerifiedOrganizer(HasRole):
    """Only verified NGO/Admin accounts may publish a campaign."""
    ...
    def has_permission(self, request, view):
        if request.method in SAFE_METHODS:
            return True
        return bool(super().has_permission(request, view) and request.user.is_verified)
